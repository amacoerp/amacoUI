...
const GenPurchaseReturn = ({ isNewInvoice, toggleInvoiceEditor }) => {
  ...
  const handleChanges = (event, newValue, index) => {
    const pD = proList?.filter(
      (obj) => obj?.product_id == newValue?.product_id
    );
  ...
  const deleteItemFromInvoiceList = (index, rId) => {
    Swal.fire({
      ...
      if (result.value) {
        if (rId) {
          url
            .delete(`delete-sales-return-detail/${rId}`)
...
>>>>>>> 9832a9e0ba62bfcd32db4a9d134f5bd7fe062f50
  ...
  const handleSubmit = () => {
    ...
    url
      .post("purchase-return-update", json)
...
>>>>>>> 9832a9e0ba62bfcd32db4a9d134f5bd7fe062f50
  ...
  const setcontact = (event) => {
    ...
    url.get(`purchase-return-data/${event.target.value}`).then(({ data }) => {
      ...
      // console.log(poN)
      setPoNumbers(
        data.getPurchaseReturnData.filter(
...
=======
              ...
              <Grid item>
                {values.status && (
                  ...
                  >
                    <MenuItem
                      onClick={() => setshouldOpenConfirmationDialogparty(true)}
                    >
                      + Add New
                    </MenuItem>
                    {contacts?.map((item) => (
                     ...
                     {!item.product[0].product_price.filter(x=>x.party.id===party_id).length?<MenuItem onClick={()=>setproductids(item.product_id,index)}><Icon>add</Icon>Add New</MenuItem>:''}
                       {item?.product[0]?.product_price.filter(x=>x.party.id===party_id).map((item, id) => (
                        ...
                        <CurrencyTextField
                          ...
                          onChange={(event, newValue) =>
                            calcualteprice(event, index, newValue)
                          }
                    ...
                    />
                      ...
                      >
                        <Icon
                          ...
                          fontSize="small"
                          onClick={() =>
                            deleteItemFromInvoiceList(index, item.prd_id)
                          }
                        >
                          delete
                        </Icon>
            ...
            >
              ...
              /> */}
                    <CurrencyTextField
                      ...
                      onChange={(e, value) => {
                        setcharge(value);
                        settotal(parseInt(value) + parseInt(subTotalCost));
                      }}
                      value={charge}
                      // value={subTotalCost?vat:parseFloat(0.00).toLocaleString(undefined,{
                      //   minimumFractionDigits:2
