...
const GenPurchaseReturn = ({ isNewInvoice, toggleInvoiceEditor }) => {
    ...
    const controlKeyPress = (e, id, nextid, prev,dropdown) => {
        ...
        if (e?.keyCode == 39) {
            ...
            } else if (nextid?.includes('purcha3se_price')) {
            ...
            if (--i >= 0) {
                ...
                } else if (r?.includes('purch3ase_price')) {
            ...
            try {
                if (r.includes('product_id')) {
                    proRef[parseInt(r)].focus();
                } else if (r.includes('purchas3e_price')) {
                    priceRef[parseInt(r)].focus();
                } else if (r.includes('invoice_no')) {
                    inputRef[parseInt(r)].focus();
            ...
            if (prev == null) {
                ...
                if (prev.includes('product_id')) {
                    ...
                    // inputRef.focus();
                } else if (prev.includes('purc3hase_price')) {
...
{CustomerList.map((item) => (
  <MenuItem value={item.id} key={item.id}>
    ...
    //   onChange={(event, newValue) => setcontact(event, newValue)}
      ...
      variant="outlined" label="Customer Name" readOnly />}
    /> */}
                                <TextField

                                    label="Customer Name"
                                    style={{ minWidth: 400, maxWidth: '400px' }}
                                    name="party_id"
                                    size="small"
                                    inputProps={{
                                        readOnly: true
                            ...
                            <Grid item>
                                {values.status &&
                                    ...
                                    >
                                        <MenuItem onClick={() => setshouldOpenConfirmationDialogparty(true)}> + Add New</MenuItem>
                                        {contacts?.map((item) => (
                                            <MenuItem value={item.id} key={item.id}>
                                                {item.fname}
                                            </MenuItem>
