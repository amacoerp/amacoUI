  ...
  useEffect(() => {
    ...
    //     {
      ...
      url.get('responseData').then(({data})=>{
        ...
        let bank_receipt=data?.Advance.filter(obj=>obj?.payment_mode=="cash" && obj?.received_by?.type=="division" && (!(obj?.received_by?.type=="division" && obj?.payment_account?.type=="division")))
        let bank_expense=data.Expense.filter(obj=>obj?.payment_type=="cash" && obj?.payment_account[0]?.type=="division")
       
        
        setopening_balance(data?.Expense.filter(obj=>(obj?.payment_type=="cash" && obj.payment_account[0]?.type=="division")&&(moment(obj.created_at).format('YYYY-MM-DD')<moment(from_date).format('YYYY-MM-DD'))).reduce((initial,cal)=> initial= initial+ parseFloat(cal.amount),0)  + data?.Advance.filter(obj=>(obj.payment_mode=="cash" && obj?.received_by?.type=="division" )&&(moment(obj?.created_at).format('YYYY-MM-DD')<moment(from_date).format('YYYY-MM-DD'))).reduce((initial,cal)=> initial= initial+ parseFloat(cal.amount),0))
...
//   function to set the To date and call the filter_data function
  ...
  const filter_data=(fDate,tDate)=>{
    ...
    let bank_receipt=response.filter(obj=>obj.payment_mode=="cash" && obj.received_by.type=="division" && (moment(obj.created_at).format('YYYY-MM-DD')>=moment(fDate).format('YYYY-MM-DD') && moment(obj.created_at).format('YYYY-MM-DD')<=moment(tDate).format('YYYY-MM-DD')))
  let bank_expense=response.filter(obj=>obj.payment_type=="cash" && obj.payment_account[0].type=="division" && (moment(obj.created_at).format('YYYY-MM-DD')>=moment(fDate).format('YYYY-MM-DD') && moment(obj.created_at).format('YYYY-MM-DD')<=moment(tDate).format('YYYY-MM-DD')))
    setopening_balance((response.filter(obj=>(obj.payment_mode=="cash" && obj.received_by.type=="division")&&(moment(obj.created_at).format('YYYY-MM-DD')<moment(fDate).format('YYYY-MM-DD'))).reduce((initial,cal)=> initial= initial+ parseFloat(cal.amount),0))-(response.filter(obj=>(obj.payment_type=="cash"&& obj.payment_account[0].type=="division")&&(moment(obj.created_at).format('YYYY-MM-DD')<moment(fDate).format('YYYY-MM-DD'))).reduce((initial,cal)=> initial= initial+ parseFloat(cal.amount),0)))
   
    // +(arr.filter(obj=>(obj.payment_type=="cash")&&(moment(obj.created_at).format('YYYY-MM-DD')<moment(from_date).format('YYYY-MM-DD'))).reduce((initial,cal)=> initial= initial+ parseFloat(cal.amount),0))

    settotalcredit(response.filter(obj=>obj.payment_mode=="cash" && obj.received_by.type=="division" &&(moment(obj.created_at).format('YYYY-MM-DD')>=moment(fDate).format('YYYY-MM-DD') && moment(obj.created_at).format('YYYY-MM-DD')<=moment(tDate).format('YYYY-MM-DD'))).reduce((initial,cal)=> initial= initial+ parseFloat(cal.amount),0))
   
    settotaldebit(response.filter(obj=>obj.payment_type=="cash" && obj.payment_account[0].type=="division" &&(moment(obj.created_at).format('YYYY-MM-DD')>=moment(fDate).format('YYYY-MM-DD') && moment(obj.created_at).format('YYYY-MM-DD')<=moment(tDate).format('YYYY-MM-DD'))).reduce((initial,cal)=> initial= initial+ parseFloat(cal.amount),0))
  ...
  return (
    <div className="m-sm-30">
      ...
      <div className={clsx("invoice-viewer py-4 pt-0", classes.CashStatement)}>
                      ...
                      >
                        ...
                        <TableBody>
                          ...
                          {arr?.sort((a, b) => new Date(a?.created_at) - new Date(b?.created_at)).map((item, index) => {
                            ...
                            {item?.description?item?.description:item?.narration}
                            </TableCell>
