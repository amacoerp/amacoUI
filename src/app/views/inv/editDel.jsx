...
const InvoiceEditor = ({ isNewInvoice, toggleInvoiceEditor }) => {
  ...
  const handleIvoiceListChange = (event, index) => {
    ...
    tempItemList.map((element, i) => {
      if (index === i) {
        ...
        // if(parseInt(element.quantity) >= parseInt(event.target.value)){
        //     console.log(parseInt(element.quantity))
        //     element['delivered_quantity'] = 
        //     element[event.target.name] = event.target.value
        // }   

        if(event.target.value > parseInt(element.quantity)){
  ...
  const handleSubmit = () => {
    ...
    arr.quotation_detail = tempItemList
    ...
    url.post('deleveryUpdate', json)
      .then(function (response) {
        ...
        routerHistory.push(navigatePath + `/invview/${id}/invoice`)
